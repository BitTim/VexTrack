...
namespace VexTrack.MVVM.ViewModel
{
	class GoalViewModel : ObservableObject
	{
		...
		public void Update(bool epilogue)
		{
			...
			GoalEntryData total = GoalDataCalc.CalcTotalGoal(TotalGoalUUID, TrackingDataHelper.CurrentSeasonData.ActiveBPLevel, TrackingDataHelper.CurrentSeasonData.CXP, epilogue);
			GoalEntryData battlepass = GoalDataCalc.CalcBattlepassGoal(BattlepassGoalUUID, TrackingDataHelper.CurrentSeasonData.ActiveBPLevel, epilogue);
			GoalEntryData level = GoalDataCalc.CalcLevelGoal(LevelGoalUUID, TrackingDataHelper.CurrentSeasonData.ActiveBPLevel, TrackingDataHelper.CurrentSeasonData.CXP);

			total.Position = 0;
			battlepass.Position = 1;
			level.Position = 2;

			BuiltinEntries.Add(total);
			BuiltinEntries.Add(battlepass);
			BuiltinEntries.Add(level);

			TrackingDataHelper.Data.Goals.OrderBy(x => x.Position);

			foreach (GoalGroup gg in TrackingDataHelper.Data.Goals)
			{
				gg.Goals.OrderBy(x => x.Position);

				List<GoalEntryData> ged = new();
				foreach (Goal g in gg.Goals)
				{
					GoalEntryData ge = GoalDataCalc.CalcUserGoal(gg.UUID, g);
					ge.Position = ged.Count;
					ged.Add(ge);
				}

				UserEntries.Add(new GoalGroupData(gg.UUID, gg.Name, ged, UserEntries.Count));
			}
