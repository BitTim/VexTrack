...
namespace VexTrack.Core
{
	...
	public class Goal
	{
		...
		public Goal(string uuid, string name, int total, int collected, string color, string dependency, bool paused, int position = -1)
		{
			UUID = uuid;
			Name = name;
			Total = total;
	...
	public class GoalGroup
	{
		...
		public GoalGroup(string uuid, string name, List<Goal> goals, int position = -1)
		{
			UUID = uuid;
			Name = name;
			Goals = goals;
	...
	public static class TrackingDataHelper
	{
		...
		public static void LoadData()
		{
			...
			foreach (JObject goalGroup in jo["goals"])
			{
				...
				foreach (JObject goal in source)
				{
					...
					{
						position = goals.Count;
						reSave = true;
					}
					else position = (int)goal["position"];

					if (uuid == null) uuid = Guid.NewGuid().ToString();
					goals.Add(new Goal(uuid, name, total, collected, color, dependency, paused, position));
				}
				...
				if (!convertToGrouped)
				{
					...
					int position;
					if (goalGroup["position"] == null)
					{
						position = goalGroups.Count;
						reSave = true;
					}
					else position = (int)goalGroup["position"];

					goalGroups.Add(new GoalGroup(uuid, name, goals, position));
				}
				else
				{
		...
		public static void SaveData()
		{
			...
			foreach (GoalGroup goalGroup in Data.Goals)
			{
				...
				goalGroubObj.Add("position", goalGroup.Position);
				...
				foreach (Goal goal in goalGroup.Goals)
				{
					...
					goalObj.Add("position", goal.Position);
		...
		public static void EditGoalGroup(string uuid, string name, int position)
		{
			Data.Goals[Data.Goals.FindIndex(gg => gg.UUID == uuid)].Name = name;
			Data.Goals[Data.Goals.FindIndex(gg => gg.UUID == uuid)].Position = position;
			CallUpdate();
		}
