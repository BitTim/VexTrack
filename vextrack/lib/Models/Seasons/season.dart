...
class Season {
  ...
  Season(this.id, this.activeLevel, this.activeXP, this.inactiveDays, this.meta,
      this.history);

  static Season fromDoc(
      DocumentSnapshot doc, SeasonMeta meta, List<HistoryEntryGroup> history) {
    return Season(
      doc.id,
  ...
  int getUserIdeal({int? offset}) {
    offset = offset ?? -2;
    offset += SettingsService.getSettingsData().bufferDays.toInt();

    int remaining = getTotal() - getXPSum();
    if (getRemainingDays() - offset <
        SettingsService.getSettingsData().bufferDays.toInt()) {
      return remaining;
    }
  ...
  int getUserEpilogueIdeal({int? offset}) {
    ...
    if (getRemainingDays() - offset <
        SettingsService.getSettingsData().bufferDays.toInt()) return remaining;
    return (remaining / (getRemainingDays() - offset)).round();
  }
  ...
  int getIdeal(int idx, bool epilogue) {
    int effectiveDuration =
        getDuration() - SettingsService.getSettingsData().bufferDays.toInt();
  ...
  bool hasEpilogue() {
    return getProgress() >= XPCalc.getMaxProgress();
  }
